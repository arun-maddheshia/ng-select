import { Directive, TemplateRef, ChangeDetectionStrategy, Component, ElementRef, Input, Injectable, Renderer2, EventEmitter, Output, ViewChild, NgZone, forwardRef, Inject, ViewEncapsulation, HostListener, ChangeDetectorRef, ContentChild, HostBinding, ContentChildren, InjectionToken, NgModule } from '@angular/core';
...
class ItemsList {
     ...
     */
    _groupBy(items, prop) {
        const /** @type {?} */ isPropFn = prop instanceof Function;
        const /** @type {?} */ groups = items.reduce((grouped, item) => {
            const /** @type {?} */ key = isPropFn ? (/** @type {?} */ (prop)).apply(this, [item.value]) : item.value[/** @type {?} */ (prop)];
